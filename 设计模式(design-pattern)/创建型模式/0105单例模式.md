...
这种方法的单例模式不是线程安全的，因为每个线程调用`getInstance()`是异步的；
所以很有可能一个线程调用了`getInstance()`来创建对象并获取，而在它判断对象是否为空的同时另一个线程也调用了`getInstance()`
这样就会创建了两个不同的对象。

### 饿汉式

这种方法是线程安全的，因为类外初始化的静态对象是在main函数执行前也就是代码执行前就初始化了，所以是线程安全的。

### 总结

多线程的常用下推荐使用饿汉式方法来定义单例类。

懒汉式也可以，但是需要额外首先线程加锁和解锁。
...
## 补充02：static对象初始化容易出现的编译错误

如果有两个以上的cpp文件include了static类外初始化对象的类定义的头文件时，一定要注意把类外初始化放到类实现的cpp文件里去

例如单例类`SingletonA`在 `SingletonA.h`定义，其类外初始化了static成员`ptr`，
然后有`main.cpp`和`test.cpp`两个cpp文件include了`SingletonA.h`，那么就会报错：ptr重复定义

正确的做法：就算你把所有的函数实现都放在了`SingletonA.h`里，也一定要把`ptr`类外初始化的语句放到`SingletonA.cpp`里去
